...
A small ComfyUI custom node package containing two switch nodes that route any value:
...
The package is registered with ComfyUI via the standard `NODE_CLASS_MAPPINGS` / `NODE_DISPLAY_NAME_MAPPINGS` re-export from `__init__.py`. It has **no Python dependencies** (`pyproject.toml` lists none beyond dev tooling) and **no frontend** — both nodes are pure Python that runs inside ComfyUI's process. `SimpleSelectorSwitch` uses ComfyUI's stock COMBO widget; no JS was added.
