...
A single-node ComfyUI custom node package. `LenientSwitch` routes any value through a switch where the **condition is decoupled from the pass-through source** — each slot (A–E) has its own optional condition input. See `README.md` for the user-facing spec.
...
All node logic lives in `lenient_switch_node.py`. Two pieces are non-obvious and easy to break:
